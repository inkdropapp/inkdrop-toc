...
function search(root, expression, settings) {
  ...
  visit(root, 'heading', function (node, position, parent) {
    const value = toString$1(node, {includeImageAlt: false});
    /** @type {string} */
...
function one(node) {
  ...
  if ('children' in node) {
    ...
    return Object.assign(structuredClone$1(copy), {
      children: all(node.children)
    })
  }

  const {position, ...copy} = node;
  return structuredClone$1(copy)
}
...
module.exports = {
  activate() {
    if (inkdrop.markdownRenderer) {
      inkdrop.markdownRenderer.remarkPlugins.push(remarkToc);
    }
  },
